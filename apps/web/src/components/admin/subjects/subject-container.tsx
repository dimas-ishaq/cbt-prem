  ...
  return (
    <Stack gap={6} bg="bg.canvas" color="text.primary" px={{ base: 4, md: 6, xl: 8 }} py={{ base: 4, md: 6 }}>
      ...
      <Box bg="bg.surface" borderRadius="card" borderWidth="1px" borderColor="border.default" shadow="card-dark" p={4}>
        ...
        <SubjectTable subjects={paginatedSubjects} filteredCount={filteredSubjects.length} currentPage={currentPage} pageSize={pageSize} onPageChange={setCurrentPage} onPageSizeChange={setPageSize} onEdit={handleEdit} onDelete={async (subject) => { const confirmed = await confirmDialog({ title: t('deleteSubjectTitle'), description: t('deleteSubjectConfirm', { name: subject.name }), confirmText: t('deleteBtn') }); if (confirmed) deleteMutation.mutate(subject.id); }} />
      </Box>
