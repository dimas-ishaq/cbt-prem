...
export default function ReportsRecommendationPage() { const { data, isLoading } = useQuery<ReportsResponse>({ queryKey: ['reports-recommendations'], queryFn: async () => { const res = await api.get('/reports/recommendations'); return res.data; }, });

  if (isLoading) { return ( <Flex justify="center" align="center" py={16}> <Spinner size="lg" color="indigo.600" /> <Text ml={3} color="gray.500">Memuat rekomendasi laporan...</Text> </Flex> ); }

  const categories = [ { key: 'exam', items: data?.exam || [] }, { key: 'student', items: data?.student || [] }, { key: 'monitoring', items: data?.monitoring || [] }, { key: 'operational', items: data?.operational || [] }, { key: 'premium', items: data?.premium || [] }, ];

  return ( <Stack gap={8}> <Box> <Heading size="xl" fontWeight="bold" color="gray.900">Rekomendasi Laporan CBT Premium</Heading> <Text color="gray.500" mt={1}>Setiap laporan dihasilkan otomatis berdasarkan data yang tersedia. Pilih kategori sesuai kebutuhan Anda.</Text> </Box> {categories.map((cat) => { if (!cat.items.length) return null; const IconComp = CategoryIcon[cat.key]; return ( <Box key={cat.key}> <Heading size="md" fontWeight="semibold" color="gray.800" mb={4} display="flex" alignItems="center" gap={2} > <Icon as={IconComp} color="indigo.500" /> {categoryLabel[cat.key]} </Heading> <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} gap={6}> {cat.items.map((r) => ( <ReportCard key={r.id} report={r} /> ))} </SimpleGrid> </Box> ); })} </Stack> );
