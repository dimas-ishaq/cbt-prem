...
export default function QuestionBankListPage() {
  ...
  const deleteBankMutation = useMutation({ mutationFn: async (id: string) => api.delete(`/question-banks/${id}`), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['question-banks'] }); toast.success('Bank soal berhasil dihapus'); }, onError: (err: any) => toast.error(err.response?.data?.message || 'Gagal menghapus bank soal') });
