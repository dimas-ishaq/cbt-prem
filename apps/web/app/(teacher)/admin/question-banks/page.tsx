    ...
    <Stack gap={6} p={6}>
      <Flex align="start" justify="space-between" gap={4} wrap="wrap">
        <Box>
          ...
          <Text color="text.secondary" mt={1}>{t('qbDesc')}</Text>
        </Box>
        <Button bg="brand.solid" color="text.inverted" _hover={{ bg: 'brand.text' }} borderRadius="lg" px={4} h={11} fontWeight="semibold" onClick={() => { resetForm(); setIsModalOpen(true); }} cursor="pointer">
          <Plus size={18} /> <Box as="span" ml={2}>{t('addQb')}</Box>
        </Button>
      </Flex>

      <Box bg="bg.subtle" border="1px solid" borderColor="border.default" borderRadius="card" p={4}>
        <Flex gap={2} align="center" color="text.secondary" mb={2}><Info size={16} /> <Text fontSize="sm" fontWeight="medium">Rekomendasi: tambah 1 bank per mapel per paket soal. Duplikat bank paling cepat buat variasi ujian.</Text></Flex>
        <Text fontSize="xs" color="text.muted">Klik nama bank buat kelola soal. Sort paling kepakai: jumlah soal.</Text>
      </Box>
      ...
      {!isLoading && banks && banks.length > 0 && (
        <Box bg="bg.surface" p={4} borderRadius="card" border="1px solid" borderColor="border.default" shadow="sm">
          <Flex direction={{ base: 'column', lg: 'row' }} gap={4} align={{ lg: 'center' }}>
            <Box flex={2} position="relative">
              ...
              <Input placeholder="Cari bank soal..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} pl={10} borderRadius="lg" borderColor="border.default" bg="bg.surface" _focus={{ borderColor: 'input.focus.border', boxShadow: '0 0 0 1px var(--chakra-colors-input-focus-border)' }} />
            </Box>
            <Flex flex={{ base: 1, lg: 3 }} gap={3} direction={{ base: 'column', md: 'row' }} width="full">
              ...
              {isFilterActive && <Button onClick={() => { setSearchQuery(''); setSelectedSubjectId(''); setSelectedCategory(''); setSortBy('name-asc'); }} variant="outline" borderColor="border.default" color="text.secondary" _hover={{ bg: 'bg.subtle' }} borderRadius="lg" height="40px" px={4} cursor="pointer" flexShrink={0}><RotateCcw size={16} style={{ marginRight: '6px' }} />Reset</Button>}
            </Flex>
          </Flex>
        </Box>
      )}
      ...
      {isLoading ? (
        ...
        <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} gap={4}>
          {sortedBanks.map((bank) => {
            ...
            return (
              <Box key={bank.id} bg="bg.surface" borderRadius="card" p={5} borderWidth="1px" borderColor="border.default" shadow="sm" _hover={{ shadow: 'md', borderColor: 'border.strong' }} transition="all 0.2s" display="flex" flexDirection="column" justifyContent="space-between" minH="170px">
                <Box>
                  <Flex justify="space-between" align="flex-start" gap={2} mb={2}>
                    <Link href={`/admin/question-banks/${bank.id}`} style={{ textDecoration: 'none' }}>
                      <Heading size="md" fontWeight="bold" color="text.primary" _hover={{ color: 'brand.text' }} cursor="pointer">{bank.name}</Heading>
                    </Link>
                    <HStack gap={1} flexShrink={0}>
                      <IconButton aria-label="Edit bank" size="sm" variant="ghost" color="text.secondary" _hover={{ bg: 'bg.subtle' }} borderRadius="lg" onClick={() => handleEdit(bank)} cursor="pointer"><Edit2 size={16} /></IconButton>
                      <IconButton aria-label="Delete bank" size="sm" variant="ghost" color="status.danger.text" _hover={{ bg: 'status.danger.bg' }} borderRadius="lg" onClick={() => handleDelete(bank.id)} cursor="pointer"><Trash2 size={16} /></IconButton>
                    </HStack>
                  </Flex>
                  <Text color="text.secondary" fontSize="sm" fontWeight="medium">{bank.subject?.name || '-'}</Text>
                  <HStack gap={2} mt={3} flexWrap="wrap">
                    <Badge bg="brand.subtle" color="brand.text" borderRadius="full" px={2.5} py={1}>{bank.category || 'Umum'}</Badge>
                    <Badge bg="status.success.bg" color="status.success.text" borderRadius="full" px={2.5} py={1}>{qCount} soal</Badge>
                  </HStack>
                </Box>
                <Flex justify="space-between" align="center" mt={4} pt={3} borderTopWidth="1px" borderColor="border.default">
                  <Text fontSize="xs" color="text.muted">{bank.updatedAt ? new Date(bank.updatedAt).toLocaleDateString('id-ID', { day: '2-digit', month: 'short' }) : 'Belum ada update'}</Text>
                  <Link href={`/admin/question-banks/${bank.id}`} style={{ textDecoration: 'none' }}>
                    <Text fontSize="xs" color="brand.text" fontWeight="semibold" _hover={{ color: 'brand.solid' }}>{t('manageQuestions')}</Text>
                  </Link>
                </Flex>
              </Box>
            );
      ...
      {isModalOpen && (
        <Box position="fixed" inset={0} bg="blackAlpha.600" backdropFilter="blur(4px)" display="flex" alignItems="center" justifyContent="center" zIndex={50} p={4}>
