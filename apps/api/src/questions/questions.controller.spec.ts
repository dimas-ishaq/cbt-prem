...
describe('QuestionsController', () => {
  const service = { create: jest.fn(), findByBank: jest.fn(), update: jest.fn(), remove: jest.fn(), removeBank: jest.fn() };
  const importService = { previewFromDocx: jest.fn(), importFromDocx: jest.fn() };
  ...
  it('create delegates', async () => {
    ...
    const mod = await Test.createTestingModule({ controllers: [QuestionsController], providers: [
