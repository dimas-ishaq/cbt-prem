...
describe('StudentsService', () => {
  ...
  describe('create', () => {
    it('should create student user with hashed password', async () => {
      mockPrisma.user.create.mockResolvedValue({
        id: 'u1', username: 'adi', student: { id: 's1', nis: '12345' },
      });
      const result = await service.create({
        username: 'adi', fullName: 'Adi', nis: '12345',
      } as any);
  ...
  describe('findAll', () => {
    it('should return paginated students', async () => {
      ...
      const result = await service.findAll(undefined, undefined, undefined, 0, 10);
    ...
    it('should filter by majorId', async () => {
      ...
      expect(mockPrisma.student.findMany).toHaveBeenCalledWith(
        expect.objectContaining({ where: expect.objectContaining({ majorId: 'm1' }) })
      );
    });
    ...
    it('should filter no-class students', async () => {
      ...
      expect(mockPrisma.student.findMany).toHaveBeenCalledWith(
        expect.objectContaining({ where: expect.objectContaining({ rombelId: null }) })
      );
    });
  });
  ...
  describe('findOne', () => {
    it('should return student by id', async () => {
      mockPrisma.student.findUnique.mockResolvedValue({ id: 's1', user: { fullName: 'Adi' } });
  ...
  describe('getMyProfile', () => {
    it('should return profile for student', async () => {
      mockPrisma.student.findUnique.mockResolvedValue({
        id: 's1', nis: '123', majorId: 'm1', rombelId: 'r1',
        user: { fullName: 'Adi', username: 'adi', email: 'a@b.com', role: 'SISWA', photo: null },
        rombel: { name: 'X RPL 1' }, major: { name: 'RPL' },
      });
  ...
  describe('updatePhoto', () => {
    it('should update user photo', async () => {
      ...
      expect(mockPrisma.user.update).toHaveBeenCalledWith({
        where: { id: 'u1' }, data: { photo: '/photo.jpg' },
      });
    });
  });
  ...
  describe('remove', () => {
    it('should delete student and user', async () => {
      mockPrisma.student.findUnique.mockResolvedValue({ id: 's1', userId: 'u1' });
