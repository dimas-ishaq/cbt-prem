...
describe('StudentProfileController', () => {
  ...
  it('uploadPhoto reject invalid mimetype', async () => {
    await expect(
      controller.uploadPhoto({ user: { userId: 'u1' } }, {
        mimetype: 'text/plain',
        size: 10,
        originalname: 'a.txt',
  ...
  it('uploadPhoto reject invalid image content', async () => {
    ...
    ).rejects.toThrow('Isi file foto tidak valid');
  });
  ...
  it('uploadPhoto accept valid jpeg', async () => {
    ...
    const result = await controller.uploadPhoto({ user: { userId: 'u1' } }, {
      mimetype: 'image/jpeg',
      size: 4,
      originalname: 'a.jpg',
      buffer,
    ...
    expect(mockService.updatePhoto).toHaveBeenCalledWith(
      'u1',
      expect.any(String),
    );
  });
