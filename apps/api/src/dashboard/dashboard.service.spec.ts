...
describe('DashboardService', () => {
  ...
  it('getStats maps avgScore and lists', async () => {
    ...
    prisma.exam.findMany.mockResolvedValue([{ id: 'e1', title: 'UTS', status: 'ONGOING', subject: { name: 'Math' }, _count: { examSessions: 4 } }]);
    prisma.violation.findMany.mockResolvedValue([{ id: 'v1', type: 'TAB_SWITCH', level: 'LOW', timestamp: new Date('2025-01-01'), examSession: { student: { user: { fullName: 'Siswa Satu' } } } }]);
    ...
    expect(stats).toEqual(expect.objectContaining({ totalStudents: 1, activeExams: 2, totalSubjects: 3, avgScore: 88 }));
