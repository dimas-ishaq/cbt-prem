...
describe('ExamSessionsService', () => {
  ...
  it('startSession reject non-student', async () => {
    ...
    await expect(
      service.startSession({ examId: 'e1' } as any, 'u1'),
  ...
  it('submitAnswer reject session not in progress', async () => {
    ...
    await expect(
      service.submitAnswer('sess1', { questionId: 'q1' } as any),
  ...
  it('finishSession reject non-IP/LOCKED status', async () => {
    ...
    await expect(service.finishSession('sess1')).rejects.toThrow(
      'Session is not in progress or locked',
    );
  });
  ...
  it('startSession rejects when no attendance check-in', async () => {
    ...
    await expect(
      service.startSession({ examId: 'e1' } as any, 'u1'),
    ).rejects.toThrow('check in before starting');
  });
  ...
  it('submitAnswer persists answer via upsert', async () => {
    ...
    const result = await service.submitAnswer('sess1', {
      questionId: 'q1',
