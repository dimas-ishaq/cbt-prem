...
describe('SubjectsService', () => {
  ...
  it('create uppercases code', async () => {
    ...
    await expect(
      service.create({ name: 'Mat', code: 'ipa' } as any),
  ...
  it('remove delete subject', async () => {
    prisma.subject.findUnique.mockResolvedValue({
      id: 's1',
      _count: { questionBanks: 0, exams: 0, teachers: 0 },
    });
    ...
    await expect(service.remove('s1')).resolves.toEqual({
      success: true,
