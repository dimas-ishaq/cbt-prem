...
export class SettingsController {
  ...
  async uploadFavicon(@UploadedFile() file: Express.Multer.File) {
    if (!file) throw new BadRequestException('File favicon wajib diunggah');
    ...
    if (file.size > 256 * 1024)
      throw new BadRequestException('Ukuran favicon maksimal 256 KB');
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        const timeout = setTimeout(
          () => reject(new Error('Koneksi Redis time out')),
          3000,
        );
      ...
      if (password)
        await this.settingsService.update('redisPassword', password);
      return { success: true, message: 'Koneksi Redis berhasil' };
    } catch (err: any) {
      throw new InternalServerErrorException(
        err?.message || 'Koneksi Redis gagal',
      );
    } finally {
