...
describe('MajorsController', () => {
  ...
  it('findOne delegates', async () => {
    ...
    const mod = await Test.createTestingModule({ controllers: [MajorsController], providers: [{ provide: MajorsService, useValue: service }] }).overrideGuard(JwtAuthGuard).useClass(AllowAllGuard).overrideGuard(PermissionsGuard).useClass(AllowAllGuard).compile();
