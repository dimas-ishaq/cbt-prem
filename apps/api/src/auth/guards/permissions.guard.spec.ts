...
describe('PermissionsGuard', () => {
  ...
  it('allow when no required permissions', async () => {
    ...
    await expect(
      guard.canActivate(ctx({ userId: 'u1', role: 'SISWA' })),
    ).resolves.toBe(true);
  });
  ...
  it('allow super admin bypass', async () => {
    ...
    await expect(
      guard.canActivate(ctx({ userId: 'u1', role: 'SUPER_ADMIN' })),
    ).resolves.toBe(true);
  });
  ...
  it('deny without permission', async () => {
    ...
    await expect(
      guard.canActivate(ctx({ userId: 'u1', role: 'GURU' })),
    ...
    prisma.userRole.findMany.mockResolvedValue([
      {
        role: {
          isActive: true,
          slug: 'teacher',
          permissions: [{ permission: { name: 'EXAM_EDIT' } }],
        },
      },
    ]);
    await expect(
