...
describe('ExamsController (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    await prisma.major.create({
      data: { id: majorId, name: 'Exam Major AA', code: 'EXM-AA' },
    });
    await prisma.rombel.create({
      data: { id: rombelId, name: 'Exam Rombel AA', majorId },
    });
    await prisma.user.create({
      data: {
        id: userId,
        username: 'exam-guru-aa@e2e.test',
        email: 'exam-guru-aa@e2e.test',
        password: pw,
        fullName: 'Exam Teacher',
        role: 'GURU',
      },
    });
    await prisma.teacher.create({ data: { id: teacherId, userId } });
    await prisma.subject.create({
      data: { id: subjectId, name: 'Exam Subject AA', code: 'EXM-SUB-AA' },
    });
    ...
    await prisma.questionBank.create({
      data: { id: bankId, name: 'Exam QB', subjectId, teacherId },
    });
    ...
    const login = await request(app.getHttpServer())
      .post('/api/auth/login')
      .send({ username: 'exam-guru-aa@e2e.test', password: 'password123' });
